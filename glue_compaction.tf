...
resource "aws_s3_object" "glue_compaction_script" {
    ...
    source = "${path.module}/../glue/glue_compaction.py"
    etag = filemd5("${path.module}/../glue/glue_compaction.py")
}
...
resource "aws_glue_job" "compaction" {
    ...
    command {
      ...
      script_location = "s3://${var.scripts_bucket}/${aws_s3_object.glue_compaction_script.key}"
      python_version = "3"
    }
